    ...
    for (int j = 0; j < width; j++) {
      if (i == 0 || i == height - 1 || j == 0 || j == width - 1) {
        ...
        for (Bullet& bullet : bullets) {
          if (bullet.isActiveStatus() && bullet.getX() == j &&
              bullet.getY() == i) {
            frame += CHAR_BULLET;
            drawn = true;
            break;
        ...
        if (!drawn && bossActive && boss.state() && boss.getX() == j &&
            boss.getY() == i) {
          frame += CHAR_BOSS;
          drawn = true;
        }
...
int main() {
  try {
    ...
    while (currentLevel <= MAX_LEVEL) {
      ...
      draw_field(bullets, enemies, spaceship, score, boss, bossActive,
                 totalKills, currentLevel);
      ...
      while (true) {
        ...
        if (command == 'a' || command == 'A' || command == 'd' ||
            command == 'D') {
