...
Enemy::Enemy(int x_, int y_)
    : x(x_), y(y_), isLive(true), hasAnimationShown(false) {}

int Enemy::getX() const { return x; }
int Enemy::getY() const { return y; }
bool Enemy::state() const { return isLive; }
...
bool Enemy::is_destroyed() const { return !isLive; }
bool Enemy::is_animation_shown() const { return hasAnimationShown; }
...
void Boss::kill() { isLive = false; }

int Boss::GetHP() const { return health; }